...
const CartScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    // console.log('cartReference: ', cartReference);
    ...
    if (user) {
      ...
      cartReference.on(
        ...
        (snapshot) => {
          ...
          console.log("--Cart snapshot length: ", snapshot.numChildren());
  ...
  function confirmOrder() {
    ...
    console.log("DELIVER HERE: ", user.location);
    ...
    array.push({
      ...
      userID: user.userID,
      time: total_duration,
      address: userInfo?.address || user.location,
  ...
  function changeQty(item, action) {
    const itemReference = firebase
      .app()
      .database(DATABASE_URL)
      .ref("/Cart/" + user.userID + '/' + item.name);
    if (action == "+") {
      itemReference
